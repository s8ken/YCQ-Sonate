...
import { Box, Button, Card, CardContent, CardHeader, Chip, Dialog, Divider, Typography, useTheme } from "@mui/material"
  ...
  return (
    <>
      <Card sx={{ mb: 3 }}>
        <CardHeader
          title="Agent Bonding"
          action={<Chip label={bondingStatus.text} color={bondingStatus.color} size="small" />}
        />
        <Divider />
        <CardContent>
          <Box sx={{ mb: 2 }}>
            <Typography variant="body1" paragraph>
              Bonding creates a deep connection between you and your AI agent, enabling enhanced context awareness and
              ethical alignment.
            </Typography>
            ...
            {agent && agent.bondingStatus === "bonded" ? (
              <Box className="bonding-cta" sx={{ p: 2, borderRadius: 2, textAlign: "center" }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  You have a strong bond with {agent.name}. This enables enhanced context awareness and ethical
                  alignment.
                </Typography>
              </Box>
            ) : (
              ...
              <Box className="cosmic-bg" sx={{ p: 2, borderRadius: 2, color: "white" }}>
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
                  <Box sx={{ minWidth: 120 }}>
                    ...
                    <Typography variant="h6">{agent.traits.ethical_alignment || 3}/5</Typography>
                  </Box>
                  <Box sx={{ minWidth: 120 }}>
                    <Typography variant="caption" sx={{ opacity: 0.8 }}>
                      Creativity
                    </Typography>
                    <Typography variant="h6">{agent.traits.creativity || 3}/5</Typography>
                  </Box>
                  <Box sx={{ minWidth: 120 }}>
                    <Typography variant="caption" sx={{ opacity: 0.8 }}>
                      Precision
                    </Typography>
                    <Typography variant="h6">{agent.traits.precision || 3}/5</Typography>
                  </Box>
                  <Box sx={{ minWidth: 120 }}>
                    <Typography variant="caption" sx={{ opacity: 0.8 }}>
                      Adaptability
                    </Typography>
                    <Typography variant="h6">{agent.traits.adaptability || 3}/5</Typography>
                  </Box>
                </Box>
              </Box>
            </Box>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        <BondingRitual agent={agent} onComplete={handleBondingComplete} />
      </Dialog>
    </>
  )
}
